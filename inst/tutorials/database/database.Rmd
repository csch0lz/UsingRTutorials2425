...
# Use View(StudentWave) if you want to view the contents of the tibble that you created.
# But note that the code checker won't work if you add this command.
...
# Use View(StudentBestfriend) if you want to view the contents of the tibble that you created.
# But note that the code checker won't work if you add this command.
...
# Use View(Neighbourhood) if you want to view the contents of the tibble that you created.
# But note that the code checker won't work if you add this command.
...
# Use View(Student) if you want to view the contents of the resulting tibble.
# But note that the code checker won't work if you add this command.
...
However, not every many-to-many relation is picked up by the pimary key. This is the next topic in this tutorial.
...
- If less then six friends were nominated, the cells contain missing vallues (`NA`). That is a waste of space.
...
__Hint:__ Use tibble `StudentWave` as your starting point. Don't forget to drop some variables that you do not need in the new tibble. What do you want to do with missing values?
</div>

```{r gatherfriends-notshown, eval=FALSE}
...
```{r gatherfriends2-notshown, eval=FALSE}
StudentWave <- StudentWave %>%
  select(student:romantic)
```
...
Tibble `GlasgowWide` contains the original panel ave data in this format. (This tibble is loaded in this tutorial.)
...
```{r owncolumn, exercise = TRUE, exercise.setup = "bestfriendstable-solution", exercise.eval = TRUE}
str(StudentBestfriend)
```
...
<div id="gatherBestfriend-hint">
__Hint:__ You have to spread variables.
</div>
...
<!-- To hide the solution, use a textual hint. -->
<div id="separate-hint">
__Hint:__ You cannot split on a particular character, such as a blank, because you would get more than two parts. Separate on position number: At which character number do you want to split?
</div>
...
gradethis::grade_code(
  correct = "", 
  incorrect = "Use the tidyverse function `separate()`."
  )
```
...
<div class="question" >
"So, how can we explain the number of cases if we use a full join on the tibbles `Student` and `StudentBestfriend`?
</div>
...
```{r crosstabs1, exercise = TRUE, exercise.setup = "studentwavetable-solution"}

```

<!-- To hide the solution, use a textual hint. -->
<div id="crosstabs1-hint">
__Hint:__ Inspect intermediary results by piping them into `View()`.
</div>

```{r crosstabs1-solution}
StudentWave %>% left_join(Student, by = "student") %>% count( sex, alcohol ) %>% pivot_wider(names_from = sex, values_from = n)
```
...
Let us go one step futher. Use your knowledge about the `kableExtra` package from Session 2.
...
__Hint:__ Use the [kableExtra webpage](https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_html.html) for the layout. Column names are partly defined in the `kable()` function and partly in a `kableExtra` function.
</div>
...
The `StudentWaveFriend` tibble contains all the information that we need to draw a friendship network for the Glasgow data. Variable `student` in this tibble represents the student nominating a friend, variable `friend` represents the student nominated, and the simple fact that the nominator and nominee are in the same bservation represents the arrow. A line in a network is just a pair of nodes!
...
net_wave1 <- graph_from_data_frame(d=arrows_wave1, vertices=nodes_wave1, directed=TRUE)

# Create a layout: positioning of the nodes.
set.seed(35546)
layout_1 <- layout_with_fr(net_wave1)
# Spread out the network horizontally.
layout_1 <- norm_coords(layout_1, ymin=-1, ymax=1, xmin=-2, xmax=2)
# Plot the network.
par(mar = c(0, 0, 0, 0))
plot(net_wave1, 
...
Include the code to check your data and the code to tidy the data in the Rmakrdown file of your group's Data Project.
