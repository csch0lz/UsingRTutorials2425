...
Note that the help file opens in your browser, not within the tutorial.
</div>
...
Glasgow %>% filter(money >= 0) %>% group_by(student, sex) %>% summarise(n_rom = sum(romantic == "yes", na.rm = TRUE)) %>% ungroup() %>% count(sex, n_rom)
```
...
- The current version of __summarise()__ by default ungroups the results. Hence the message in the console "`summarise()` ungrouping output (override with `.groups` argument)". 
...
Finally, set `-1` on variable `money` to missing. A negative number of pounds as pocket money cannot be correct.
</div>
...
Glasgow %>% mutate(money = na_if(money, -1))
```
...
- Use `count()` (which is equal to `group_by()` and `summarize(n = n())`) to better understand a variable or a combination of two or more variables.
...
- In R, __c()__ creates a __vector__, which is a series of values (of the same type). It is used when we have to pass more than one value to a function. For example, __ggplot()__ uses it a lot.
</div>
...
<div class="question" >
We don't like guesses; we want to be sure that the variable type is correct. Check the variable types reported when the data was read. Then use the `col_types` argument in your `read_csv()` command to read the data with the right variable types. 
</div>
...
- Don't worry about code or output created by code that you may not need in the end. I is easy to skip a code chunk and not displaying its results by setting the code chunk option `eval=` to `FALSE`. Preserving (but hiding) unnecessary code prevents you from creating the code again later on.
</div>
