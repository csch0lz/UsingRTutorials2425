...
If the tidyverse package is not installed, install it now. Use `Install Packages...` from the `Tools` menu or run the command `install.packages("tidyverse")` in the _Console_ tab of the RStudio interface. Note the quotes: _tidyverse_ is the name of a file here.

Tip: During install, you may be asked if you want to install sources that need compilation. Answer __No__.

### Load the tidyverse package

We have to install a package only once, but we have to load it in our R session before we can use it. Load the most common tidyverse packages by loading the tidyverse package.
...
You cannot load a package from this tutorial. If you have the correct command, copy it to the RStudio Console and run it there by pressing Enter.

**Tip**: If a package is loaded, its box in the Packages panel is checked.

**Tip**: You can run a command also by putting the cursor somewhere within the command and press Ctrl-Enter (Windows) or Cmd-Return (Mac).

Go back to this tutorial when you have checked that the `tidyverse` package is loaded.
...
Load the data file named `http://82.196.4.233:3838/www/consumers.csv` directly from the internet into a data object called `consumers` in R. 
...
__Q1. How many variables does the data set contain?__
...
__Q2. How many numerical variables does the data set contain?__
...
__Q3. What are the first names of the first and twentieth person in the data set?__
...
gradethis::grade_result(
  pass_if(
    function(x) {stringr::str_detect(x, "Asleigh") &
        stringr::str_detect(x, "Thaamira")}, 
    "The names are indeed Asleigh and Thaamira."),
  fail_if(function(x) {stringr::str_detect(x, "Asleigh")}, 
    "Asleigh is correct, but the name of the twentieth person is missing or wrong. Use the spreadsheet button in the row of the Environment tab to view the data."),
  fail_if(function(x) {stringr::str_detect(x, "Thaamira")}, 
    "Thaamira is correct, but the name of the twentieth person is missing or wrong. Use the spreadsheet button in the row of the Environment tab to view the data."),
...
Function arguments specify the input for a function. Arguments can be named (more typing, less confusion) and unnamed (less typing, more confusion). 
...
- Look up the arguments of the function in the Help page or from the code completion options shown when you start typing within the brackets.
...
__Hint__: In the RStudio interface, put your cursor in the name of a function and press F1 to get help.
...
In the previous exercises, the function results are send to the screen. Now, store the results of the three functions below in data objects named `my_output`, `my_Output`, and `my_output` respectively.
...
If your code is correct, copy and paste it to the RStudio console to create the data objects there. Now, you can inspect them in the __Environment__ tab. What does the data object `my_output` contain? Enter the value below.
...
gradethis::grade_result(
  ...
  fail_if(~ TRUE, "Check out the data objects that you created in the RStudio interface.")
)
```
...
Use the consumers data file to create the plot shown below. Mind the details! Use the [Data Visualization Cheatsheet](https://github.com/rstudio/cheatsheets/raw/master/data-visualization-2.1.pdf) if you want to look up options, for example, which layer to use for setting the title and axis labels.
...
Create separate regression lines of different colours for females and males. Use shape instead of color to distinguish between respondents who heard about the brand by word of mouth and those who did not.
...
```{r plot_adapt-check}
gradethis::grade_code(
  correct = "Within the `aes()` function, we can link a (categorical) variable to a particular aesthetic, in this example, to color. Each group then receives its own color and its own regression line. Now, we use color as an aesthetic, not as a constant ('black')."
  )
```
...
Use the [Data Visualization Cheatsheet](https://github.com/rstudio/cheatsheets/raw/master/data-visualization-2.1.pdf) to find the geoms that are appropriate for the following plots of the `consumers` data.
...
We may want to know who is the outlier in the plot, having an exceptionally low exposure score. Use a geom to add the first names to the plot, as shown below.
...
Actually, we can draw all kinds of shapes on the plot, for example, an arrow drawing attention to the extremely low exposure score. Use `geom_segment` to create this plot.
...
1. The data visualization is sufficiently complex to tell a story. For example, it presents both a pattern (as a first impression) and deviations from this pattern (inviting reflection about the pattern) without being overwhelming because of too much information.

2. The data visualization is self-explanatory. It should be comprehensible without reading the accompanying text. For example, use informative, readable labels.
...
output$regPlot <- renderPlot({
  ...
  else if ((input$slider > 0 & input$slider < 3) | (input$slider > 10)) {
...
The code below creates an animated gif using [Gapminder](https://www.gapminder.org/) data on life expectancy, GDP per capita, and population size by country. Play around with the settings that are specific to `animate()`.

Note that it takes quite some time to generate the animation.

```{r gganimate1, exercise=TRUE, eval=FALSE}
...
animate(
  ...
  nframes = 78, #1 frame for each year from 1952 to 2007 plus 2x11  additional frames for 
...
- Carefully inspect the results created by packages that do a lot of work for you.
</div>
...
The `plotly` library is designed for creating interactive graphics. It has its own language for creating graphs but for the `ggplot` user, it provides a function to change a `ggplot` plot into an interactive `plotly` plot.

The `plotly` library offers the option to zoom, select items in the graph, and see additional information about the items in the graph. Install the packages if necessary, and run the code to see the graph. If you carefully position your cursor on a dot, the first name will pop up.

```{r plotly1, exercise=TRUE}
...
Check out the [Shiny demos gallery](https://shiny.rstudio.com/gallery/#demos) for inspiration. Start simple! First create your plot with `ggplot()`, then add it to the Shiny app.

## Data Project

Plots are very useful for getting a first idea of your data. With the members of your group, use `ggplot` plots to describe variables and relations between variables in (one of) your Data Project file(s). 

Download the Data Project data file(s) to a directory that you create for your Data Project. In the context of this tutorial, you must load the data in each code box (in RStudio, you only load it once).

As long as you do not press the *Start Over* button, the code (and plots) are preserved in this tutorial. 
...
- Don't delete plots that turn out to be useless. Comment them out (add # to each line start) and add a comment explaining why it is useless. Otherwise, someone else is going to redo your work.
</div>

```{r DataProject2, exercise = TRUE}
# Load your data.
dataName <- read_csv()
