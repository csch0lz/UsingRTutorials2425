...
consumers <- UsingRTutorials::consumers #  # readr::read_csv("http://82.196.4.233:3838/www/consumers.csv") # readr::read_csv("data/consumers.csv")
```
...
**Hint:** Unsure about a function name? Start with inspecting the cheat sheets that are available from the RStudio's *Help > Cheat Sheets* submenu. If you cannot find the right cheat sheet here (you now need the *Data Import* cheat sheet), the *Browse Cheat Sheets* option in the submenu sends you to the web page with cheat sheets.
</div>
...
__Hint:__ Use the Help page for this function (`?seq`). The help will show in the _Help_ tab in RStudio.
</div>
...
```{r cleanUp2, exercise = TRUE, exercise.setup = "output1-solution"}
...
__Hint:__ The answer checker will first check if all layers are included. If a layer is missing, it will report "I expected you to call + where you called ggplot()". Next, it will check the details within each layer. For optimal use of the checker, follow this approach. In addition, it is more efficient to set the x and y aesthetics in the `ggplot()` function than in the separate geoms, if several geoms use the same x and y aesthetics.
</div>
