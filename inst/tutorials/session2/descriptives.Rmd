...
```{r summary1-hint-4}
# Use mutate() to calculate the cumulative
# percentages.
Glasgow %>%
  group_by( ??? ) %>%
  summarise(
...
# If we calculate cumulative percentages within summarise(),
# the data are grouped, so cumsum() starts anew for every
# friendships value.
```
...
Glasgow <- Glasgow %>% 
  mutate(
    money_class = ntile(x = money, n = 3),
    money_class2 = case_when( money == 0 ~ 0, money > 0 & money <= 10 ~ 1, money > 10 ~ 2)
    )
```

<!-- To hide the solution, use a textual hint. -->
<div id="recodemoney1-hint">
__Hint:__ You should be able to complete this code.
</div>

```{r recodemoney1, exercise = TRUE, exercise.setup = "recodemoney1-ini"}
# Complete this code: replace '???'.
# Note: This code uses a version of data set Glasgow that includes the new variable money_class2.
Glasgow %>%
  #select the money variable and the new variable money_class2.
  select( ??? ) %>%
  #keep only rows with value -1 for money
  filter(money == ???)
...
If you succeeded in completing the above code correctly, you can see that a value not covered by a `case_when()` function are set to missing (`NA`).
