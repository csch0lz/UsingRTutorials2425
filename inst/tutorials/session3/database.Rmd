...
-   __student__: has an age, sex, an be a friend, and so on.
...
3.    Use the function `anyDuplicated()` to check if there is a case (row) with the same values on the selected variables as a preceding case.
      + If there is a duplicate, it is not a pruimary key; add a variable to the key (go to Step 1).
...
__Hint:__ Add the variables in the select() function that you think represent different types of cases (unit).
</div>
...
Student <- GlasgowFriends %>% 
  ...
  distinct()
# Use View() within the pipe if you want to view the contents of the tibble that you created.
```
...
-   Gather (stack) repeated information. 
...
__Hint:__ Use help on the gather() function   .
</div>
...
<div id="gatherfriends-hint">
__Hint:__ How about the select() function?
</div>
...
-   `gather()`: 
    ...
    +   Can spread more than one variable (often in panel survey data).
-   `spread()`: New function `pivot_wider()`.
...
## Become an Expert: Evaluate Your Complex Data
...
A joined tibble has more cases than the largest tibble used for the join, if:

A.    The key used to join the tibbles is _not_ a primary key in _both_ tibbles.  
B.    The join key has missing values in at least one tibble.  
C.    Not every value of the join key in one tibble appears in the other tibble.  

Check if these circumstances apply to the tibbles that you want to join. 

-   If not, the join should create a tibble with as many rows as one of the tibbles that were joined. 
-   Else, the joined tibble may contain (many) more rows than any of the two original tibbles.

### Ad A. Checking if the key is primary

You have learned earlier in this tutorial how to see if a key is a primary key. 
...
<div class="question" >
Count the number of missing values in the primary key variables (`student` and `bestfriend`) of tibble *StudentBestfriend*.
</div>
...
```{r join1b-hint-2}
# For the StudentBestfriend tibble, replace .key. by one the join key variable(s).
StudentBestfriend %>% 
  summarise(
    n_missing_student = sum(is.na( .key. )),
    n_missing_bestfriend = sum(is.na( .key. ))
    )
```

```{r join1b-hint-3}
# It is best to count the cases where at least one of the two primary key variables is missing.
# You can do this with logical or (|).
```

### Ad C. Checking join key overlap: `anti_join()`

`inner_join()` may yield fewer cases than in either of the joined tibbles:

-   If not every value of the join key in one tibble appears in the other tibble.
-   Example: Not all students appear in tibble `StudentBestfriend` if some students did not mention a best friend.
...
<div class="question">
Use the `anti_join()` function to find the students registered in tibble *Student* but not in tibble *StudentBestfriend* or the other way around.
...
If the join key is not a primary key in both tibbles:

-   The number of cases in the resulting tibble is larger than the number of cases in any of the two original tibbles;
-   The number of cases can blow up enormously.
...
<div class="question" >
How many rows (observations) do we get when we (left) join the `StudentBestfriend` tibble to the `StudentWave` tibble? Can you explain this number?
</div>

```{r joinnonprimary, exercise = TRUE, exercise.setup = "bestfriendstable-solution"}
# Send the result to the screen, so you can view it.
```

```{r joinnonprimary-hint-1}
# Start with piping the StudentWave tibble into the left_join() function.
StudentWave %>%
  left_join()
```

```{r joinnonprimary-hint-2}
# Join with the StudentBestfriend tibble. Figure out by which key variable.
StudentWave %>%
  left_join(StudentBestfriend, by = )
```

### Checking the join afterwards

Did the join do what you wanted it to do?

-   Check the number of rows and try to explain this number. Should it be smaller, equal, or larger than the number of rows in the original tibbles?
-   Check what happened to observations that have no match in the other tibble.
-   Compare some special cases in the joined tibble with their rows in the original tibbles:
    +   Cases with a missing value on a primary key variable. Each missing value is liked to each missing value on the key variable in the other tibble!
    +   Primary key values that are duplicated (several times) in the joined tibble.
...
Chapter 11 {14}: Strings. Read to see the possibilities, so you know where to
find commands that are relevant to your Data Project.

Chapter 12 {15}: Factors. Study carefully. Categorical variables are used as
factors in statistical analyses.

Chapter 13 {16}: Dates and Times. Study carefully, especially if your complex
