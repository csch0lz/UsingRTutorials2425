...
The resulting number of observations does not equal the number of observations in tibble `StudentBestfriend` with a left or right join with the order of tables reversed or a full join. Now, the students from the `Student` tibble that do not appear in the `StudentBestfriend` tibble are preserved. A full join, for example:
...
Can you explain the number of cases if we use a full join on the tibbles `Student` and `StudentBestfriend`? This is not always super simple.
