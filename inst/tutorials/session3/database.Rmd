...
Every variable in your primary key and every combination of these variables may represent a different type of cases (unit). 
...
```{r hoodtable, exercise = TRUE, exercise.lines = 8}
...
```{r student2table, exercise = TRUE, exercise.lines = 4}
...
GlasgowWide <- readr::read_csv("data/Glasgow.csv") %>% # UsingRTutorials::Glasgow %>%
...
* Compare the number of cases with those of the `Student` table (`r nrow(Student)` rows) and the `StudentBestfriend` table (`r GlasgowFriends %>% 
  select(student, bestfriend, bfperiod, bfwave ) %>% distinct() %>%
  filter(!is.na(bestfriend)) %>% 
  pivot_wider(id_cols = student:bestfriend, names_from = bfperiod, values_from = bfwave) %>% nrow()` rows). What do you notice?
</div>
...
Here are some graphical examples of what happens with the four different joins.
...
Each row (case) in the first (left) tibble is preserved in the resulting tibble. In addition, each row in the second (right) tibble with a key match in the first tibble is preserved.
...
Two of the joins produce a tibble with more cases than the largest tibble used for the join. This may happen in one or more of the following circumstances:

1. The key used to join the tibbles is _not_ a primary key in _both_ tibbles.
2. The join key has missing values in at least one tibble.
3. Not every value of the join key in one tibble appears in the other tibble.

It is important to check whether these circumstances apply to the tibbles that you want to join. If not, the join should create a tibble with as many rows as one of the tibbles that were joined. Else, the joined tibble may contain (many) more rows than any of the two original tables.

### Checking if the key is primary

You have learned earlier in this tutorial how to see if a key is a primary key. The questions below check your understanding. Use the code box below the questions if you want to use code to find the answers to the questions.

```{r joinquiz}
quiz(
  caption = "",
  question("Is the key used to join the tibbles NOT a primary key in both tibbles?",
  ...
  question("Does every value of the key used to join the tibbles appears in both tibbles?",
    ...
    answer("No", correct = TRUE, message = "You can check this with the anti_join() function, which is discussed next in this tutorial.")
  ),
...
```{r join1a, exercise = TRUE}

```

### Checking for missing values on a join key
...
```{r join1b-hint-2}
# For the Student tibble, replace .key. by the join key variable(s).
Student %>% summarise(n_missing = sum(is.na( .key. )))
```

```{r join1b-hint-3}
# Do the same for the StudentBestfriend tibble.
```
...
Variable `student` is a primary key without missing values in tibble `Student`. Therefore, adding the information from this tibble to the `StudentBestfriend` tibble (with `student` as join key) adds one neighbourhood code, one age, sex, and so on to each observation in tibble `StudentBestfriend`. The resulting number of observations equals the number of observations in tibble `StudentBestfriend`. This happens, for example in the left and right joins illustrated below. 

- A left join that joins `Student` to `StudentBestFriend`:
    `StudentBestFriend %>% right_join(Student, by = "student")`
- A right join that joins `StudentBestFriend` to `Student`:
    `Student %>% right_join(StudentBestfriend, by = "student")`
    
```{r out.width="80%"}
knitr::include_graphics("images/LeftRightjoin.png")
```

The resulting number of observations does not equal the number of observations in tibble `StudentBestfriend` with, for example, a full join. Now, students from the `Student` tibble that do not appear in the `StudentBestfriend` tibble are preserved.
...
If the join key is not a primary key in both tibbles, the number of cases in the resulting tibble is larger than the number of cases in any of the two original tibbles.
...
It is very hard to predict the number of cases that will result from a join on two keys that are not primary keys. 
...
1. Does the data set contain info on more than one type of cases (unit)?
    Or: Are some variable combinations always the same?
    If so, transfer the info on each type of cases (unit) to a separate tibble.
