...
-   *Problem set 1: PART 2 (15min)*\
-   General feedback for problem sets
-   Tidy data\
...
-   Download the Problem Set 1 .qmd file you submitted to Canvas.

-   Every exercise has a table underneath it which is meant to be used
    for grading. Most tables just have one row, called "Code". There are
    2 exercises with tables that have 2 rows, namely "Code" and
    "Explanation".

-   For the two exercises requiring an "Explanation", comment your code
    inside the code chunk (use \#) to describe:

    -   

        1)  What?: Accurately and precisely, describe what operations
            the code performs

        2)  Why?: Briefly explain why you perform this operation (e.g.
            how this operation contributes to your overall goal towards
            solving the current problem).

-   Done? Submit the commented document to the Assignment called
    **"Problem Set 1: Explanation"**

## General Feedback Problem Sets

-   Once you load `tidyverse`, all core packages are loaded, too.

-   Use `tidyverse` code whenever possible.

-   Use R code to fully produce the answer to each exercise and only
    return the answer, not irrelevant information.

-   Don't print datasets to the screen unless required for the response
    to the exercise.

-   Don't include `install.packages()`, unless you are working with an
    exotic package Xin and I may not have or know.

-   The last exercise in each problem set is usually about your Data
    Project data.

## A Word About AI

It's a great coding too - but not just yet :).
...
::: emphasis
HOMEWORK BEFORE SESSION 4: Become an expert in 1 out of the 4 tidy data
principles.
:::
...
-   Produce 2 PowerPoint slides (use the template in Session 4 module on
    Canvas).
...
### Who becomes in expert in what? 

see [Canvas Discussion
Forum](https://canvas.uva.nl/courses/40018/discussion_topics/788605)
...
to draw a friendship network for the Glasgow data:
...
Talking about networks: A relational database is a network of tables
(tibbles) and joins between tables. To the best of my knowledge,
packages for network analysis cannot display tables. The R package
`DiagrammeR`, which uses [graphviz](http://www.graphviz.org/), can do
this.
