...
If we want to use data type `difftime` in a plot, we have to change it into a numeric variable with the `as.numeric()` function.

<div class="question" >
Plot variable `lapse` (time between subsequent posts in seconds) as a histogram with 50 bins.
</div>

```{r plotlapse-ini}
Brexit <- Brexit %>% 
  mutate(pubDateTime = parse_datetime(pubDate, format = "%B %d, %Y %H:%M:%S")) %>%
  arrange(pubDateTime) %>% 
  mutate(lapse = pubDateTime - lag(pubDateTime, n = 1L))
```


```{r plotlapse, exercise = TRUE, exercise.setup = "plotlapse-ini"}

```

<div id="plotlapse-hint">
__Hint:__ You should really be able to create this plot by yourself. Just don't forget to use `as.numeric(lapse)` instead of `lapse`.
</div>
...
varnames <- data_frame(name = names(surveyweek1)[4:10]) %>%
