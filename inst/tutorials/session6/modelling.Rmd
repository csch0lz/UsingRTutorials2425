...
You already know how to execute an independent-samples _t_ test (Session 5). There are different versions of the same function for different _t_ tests.
...
In contrast to SPSS, R only gives you what you ask for. If you ask for a _t_ test, you get a _t_ test but not checks on assumptions. You have to apply those checks yourself.

You may remember that the version of the independent samples _t_ test that we must use depends on whether the population variances are equal for the two groups. 
...
In the R code for an independent samples _t_ test, the `var-equal` argument must be set to `TRUE` or `FALSE`. We can use the p value provided by `var.test()` as the value of this argument, so we include the equal variances test in the _t_ test.
...
The code chunk in the RMarkdown document:
...
```{r broom2-solution, exercise.reveal_solution = FALSE}
...
In case you later have SPSS data that you want to analyze in R: Two options for importing SPSS data.
...
<div class="question">
Import the SPSS file and have a look at it.
</div>

```{r importSPSS, exercise = TRUE}
# The SPSS system is available in the data directory of this tutorial.
...
```{r sex_fct-solution, exercise.reveal_solution = FALSE}
